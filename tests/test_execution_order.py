...
def test_optional_real_m580_safety_execution_order(filename):
    ...
    # Program-scoped FBD diagrams. Resource variables, then Tier 1 binary
    # expressions, made more pins bind, letting the unlinked-shared-variable
    # guard see them: 9 diagrams whose blocks exchange values through a shared
    # variable with no covering linkFB wire correctly stop claiming an order
    # (22/22 resolved earlier only because those pins were unbound/unresolved
    ...
    unlinked_dataflow = {"Sim_FBD", "Sim_W505", "MAV10EA100", "MAK10EA100", "MAX10EA100",
                         "MAL10EA100", "MKA10EA100", "TRIP_TG", "Séquence"}
    ...
    # 3 DFB bodies plus the 9 program diagrams named above. "Séquence" (like
    # P_MUX3 already did) gets both codes: ambiguous from the multi-writer
    # guard, and unresolved_block_order from the final unresolved-diagram sweep.
    assert codes == {"ambiguous_block_order": 2, "unresolved_block_order": 12}
